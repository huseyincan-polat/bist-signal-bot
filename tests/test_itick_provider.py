...
def make_itick_provider() -> ITickRealTimeProvider:
    config = AppConfig(
        ...
        itick_symbols=("THYAO", "GARAN", "AKBNK"),
        itick_region="TR",
    )
...
def test_itick_subscription_uses_documented_turkish_symbol_format() -> None:
    ...
    assert provider.subscription_payload() == {
        "ac": "subscribe",
        "params": "THYAO$TR,GARAN$TR,AKBNK$TR",
        "types": "quote,tick,depth",
    }


def test_itick_config_has_only_a_bist30_watchlist() -> None:
    config = load_config("config.yaml")
    assert len(config.itick_symbols) == 30
    assert set(config.itick_symbols) < set(config.symbols)
    assert {"THYAO", "GARAN", "AKBNK", "EREGL", "KCHOL", "BIMAS", "TUPRS", "ASELS"} <= set(config.itick_symbols)
...
def test_itick_health_check_requires_fresh_live_tick() -> None:
    ...
    quote = provider.parse_message(
        {"code": 1, "data": {"s": "GARAN", "r": "TR", "ld": 100, "v": 1000, "t": int(datetime.now(UTC).timestamp() * 1000), "type": "quote"}}
    )
