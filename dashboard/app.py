    ...
    def _row(self, symbol: str, global_age: float | None) -> dict[str, Any]:
        ...
        if not signal:
            return {
                ...
                "score": None, "rsi": None, "macd": None, "adx": None, "atr": None, "relative_volume": None,
                "trend": "—", "tick_momentum": None, "stop": None,
                "target": None,
                "data_age": live_age,
        ...
        return {
            ...
            "macd": metric.get("macd"), "adx": metric.get("adx"), "atr": metric.get("atr"),
            "relative_volume": metric.get("relative_volume"), "trend": metric.get("trend"),
            "tick_momentum": metric.get("tick_momentum"),
            "stop": signal.stop,
...
<div class="toolbar" id="filters"><input id="search" type="search" placeholder="Sözleşme ara (ör. BTCUSDT)" aria-label="Sözleşme ara"><span class="muted">Görünüm:</span><button class="active" data-filter="TUMU" id="filter-all" title="HAM LİSTE">TÜMÜ</button><button data-filter="ACTIVE">LONG / SHORT</button><button data-filter="GÜÇLÜ LONG">GÜÇLÜ LONG</button><button data-filter="LONG">LONG</button><button data-filter="SHORT">SHORT</button><button data-filter="GÜÇLÜ SHORT">GÜÇLÜ SHORT</button></div>
<p id="view-hint" class="view-hint">HAM LİSTE — tüm sözleşmeler (BEKLE dahil)</p>
<div class="table-wrap"><table><thead><tr><th>Sözleşme</th><th>Fiyat</th><th>Değişim %</th><th>Sinyal</th><th>Score</th><th>RSI</th><th>MACD</th><th>ATR</th><th>ADX</th><th>Trend</th><th>Tick İvme</th><th>Stop</th><th>Hedef 1</th><th>Veri Yaşı</th></tr></thead><tbody id="rows"><tr><td colspan="14" class="empty">Futures sözleşmeleri yükleniyor…</td></tr></tbody></table></div>
</main><footer class="site-footer" role="contentinfo">Hüseyin Can Polat tarafından yapılmıştır</footer><script>
let rows=[],filter='TUMU',query='',currentStatus={};const f=n=>n==null?'—':Number(n).toLocaleString('tr-TR',{maximumFractionDigits:2});const formatPrice=n=>{if(n==null)return'—';const v=Number(n),a=Math.abs(v);let d=2;if(a<1)d=4;else if(a<1000)d=3;return v.toLocaleString('tr-TR',{minimumFractionDigits:d,maximumFractionDigits:d})};const esc=s=>String(s??'—').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const ageCell=r=>r.row_data_state==='REAL_TIME'?`${f(r.data_age)} sn · CANLI`:r.row_data_state==='STALE_DATA'?`${f(r.data_age)} sn · BAYAT`:r.row_data_state==='PRIMED'?'GEÇMİŞ VERİ · SIRADA':'SIRADA';const priceCell=r=>{if(r.price==null)return'—';let c=Number(r.price_change||0),p=Number(r.price_change_pct||0),cls=c>0?'up':c<0?'down':'flat',arrow=c>0?'▲':c<0?'▼':'●',sign=c>0?'+':'';return `<b class="${cls}">${formatPrice(r.price)} USDT</b>${r.prev_close==null?'':`<br><small class="${cls}">${arrow} ${sign}${formatPrice(c)} USDT (${sign}%${f(p)})</small>`}`};const OPPORTUNITY=new Set(['ACTIVE','LONG','GÜÇLÜ LONG','SHORT','GÜÇLÜ SHORT']);
...
function filterRows(allRows){if(filter==='TUMU')return allRows;if(filter==='ACTIVE')return allRows.filter(r=>['LONG','GÜÇLÜ LONG','SHORT','GÜÇLÜ SHORT'].includes(r.signal));return allRows.filter(r=>r.signal===filter)}
function emptyMessage(){if(filter==='TUMU')return 'Bu arama için sözleşme yok.';return 'Bu filtrede aktif varlık yok, ham liste için Tümü sekmesine geçin.'}
function render(status){currentStatus=status;rows=status.rows||rows;const allBtn=document.querySelector('#filter-all');if(allBtn)allBtn.textContent=`TÜMÜ (${rows.length})`;const hint=document.querySelector('#view-hint');if(hint)hint.textContent=filter==='TUMU'?'HAM LİSTE — tüm sözleşmeler (BEKLE dahil)':'Fırsat filtresi aktif — ham liste için Tümü sekmesine geçin';document.querySelector('#provider').textContent=status.provider;document.querySelector('#connection').textContent=status.connected?'BAĞLI':'BAĞLANTI YOK';document.querySelector('#regime').textContent=status.market_regime;document.querySelector('#coverage').textContent=`${status.symbols_received}/${status.symbols_expected}`;document.querySelector('#age').textContent=status.data_age==null?'—':`${status.data_age} sn`;document.querySelector('#engine').textContent=status.ready_for_signals?'AKTİF':'EMNİYET KİLİDİ';document.querySelector('#warning').classList.toggle('hide',status.data_state==='REAL_TIME');document.querySelector('#buys').innerHTML=status.top_buys.length?status.top_buys.map(leader).join(''):'<span class="muted">Uygun LONG sinyali yok.</span>';document.querySelector('#sells').innerHTML=status.top_sells.length?status.top_sells.map(leader).join(''):'<span class="muted">Uygun SHORT sinyali yok.</span>';let base=filterRows(rows);let visible=base.filter(r=>`${r.symbol} ${r.name}`.toLocaleLowerCase('tr-TR').includes(query));document.querySelector('#rows').innerHTML=visible.length?visible.map(r=>`<tr><td><b>${esc(r.symbol)}</b><br><small class="muted">${esc(r.name)}</small></td><td>${priceCell(r)}</td><td>${r.price_change_pct==null?'—':`${r.price_change_pct>0?'+':''}%${f(r.price_change_pct)}`}</td><td class="signal ${cls(r.signal)}">${esc(r.signal)}</td><td>${f(r.score)}</td><td>${f(r.rsi)}</td><td>${f(r.macd)}</td><td>${f(r.atr)}</td><td>${f(r.adx)}</td><td>${esc(r.trend)}</td><td>${r.tick_momentum==null?'—':`${r.tick_momentum>0?'+':''}${f(r.tick_momentum)}%`}</td><td>${formatPrice(r.stop)}</td><td>${formatPrice(r.target)}</td><td>${ageCell(r)}</td></tr>`).join(''):`<tr><td colspan="14" class="empty">${emptyMessage()}</td></tr>`}
